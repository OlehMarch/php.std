<?php
	...
	class Image {
		...
		public static function loadImage() {
			$im = $_GET["im"];
			if (!$im) return false;
			if (!file_exists("images/$im")) return false;
		...
		public function getTextError($code) {
			if ($code == 1) return "Неверный тип изображения";
			elseif ($code == 2) return "Превышен максимальный размер загружаемого файла (1 МБ)";
			elseif ($code == 3) return "Ошибка при загрузке. Попробуйте еще раз.";
		}
	}
?>